...
func (a *ServerAPI) Action(w http.ResponseWriter, r *http.Request) {
	...
	if err := a.Servers.Action(*target, action); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	http.Redirect(w, r, "/servers/"+target.UUID, http.StatusSeeOther)
}
...
func (a *ServerAPI) ServerPage(w http.ResponseWriter, r *http.Request) {
	...
	uuid := strings.TrimPrefix(r.URL.Path, "/servers/")
	...
	tmpl, err := template.ParseFiles(
		"panel/web/templates/server.html",
	)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
...
func (a *ServerAPI) FilesAPI(w http.ResponseWriter, r *http.Request) {
	...
	uuid := strings.TrimPrefix(r.URL.Path, "/api/servers/")
	uuid = strings.TrimSuffix(uuid, "/files")
