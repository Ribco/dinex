...
SELECT id, user_id, uuid, node_id, external_id, name, description, created_at
FROM servers
ORDER BY id DESC
`)
	if err != nil {
...
SELECT id, uuid, node_id, external_id, name, description, created_at
FROM servers
WHERE uuid = ?
`, uuid).Scan(
		&s.ID,
...
func (m *Manager) UpdateSettings(
	...
	resp, err := m.request(
		node,
		http.MethodPut,
		"/api/v1/servers/"+server.ExternalID+"/settings",
		body,
	)
	if err != nil {
		return err
	...
	_, err = m.DB.SQL.Exec(`
UPDATE servers
SET name = ?, external_id = ?, description = ?
WHERE id = ?
`,
		name,
		externalID,
...
func (m *Manager) RenameFile(server Server, oldPath, newPath string) (int, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		b, _ := io.ReadAll(resp.Body)
		return resp.StatusCode, fmt.Errorf("agent returned HTTP %d: %s", resp.StatusCode, strings.TrimSpace(string(b)))
	}
	return resp.StatusCode, nil
}
